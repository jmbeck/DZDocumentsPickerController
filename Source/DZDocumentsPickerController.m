...
- (void)segmentAction:(id)sender
{
    ...
    DLog(@"currentSegment = %d",currentSegment);
...
- (UITableViewController *)buildTableViewControllerWithTitle:(NSString *)title
{
    ...
    EGORefreshTableHeaderView *headerView = [[EGORefreshTableHeaderView alloc] initWithFrame:rect];
...
- (void)navigationController:(UINavigationController *)navigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    if (currentSegment == 1)
    {
        if (viewController.view.tag < [depthLevel intValue])
        {
            ...
            DLog(@"PopItem && depthLevel = %d",[depthLevel intValue]);
            DLog(@"[cloudFilesDict count] = %d",[cloudFilesDict count]);
...
- (void)servicesManager:(DZServicesManager *)manager didLoadFiles:(NSArray *)files
{
    ...
    if (!isReloading)
    {
        ...
        DLog(@"depthLevel = %d",[depthLevel intValue]);
